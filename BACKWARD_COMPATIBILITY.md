...
- **Routing lines from `om-brainstorm`** (`Next: none` | `Next: om-<skill> <args>`, plus `Brief: <repo-relative path>` when a handoff brief was written) — emitted at the end of its final report, parsed by session orchestrators to route the follow-up run. Additive; no existing consumer changes.
